...
router.get('/best-profession', validateDates, async (req, res) => {
  try {
    ...
    const professionEarnings = await Profile.findOne({
      subQuery: false,
      attributes: ['profession', [sequelize.fn('SUM', sequelize.col('Contractor.Jobs.price')), 'professionEarning']],
      include: [
        {
          model: Contract,
          as: 'Contractor',
    ...
    return res.json({
      profession: professionEarnings.profession,
      totalEarned: professionEarnings.get('professionEarning') ?? 0,
    })
  } catch (error) {
    console.error(error)
